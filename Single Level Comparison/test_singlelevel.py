...
from singlelevel_mt import singlelevel_mt
import time
...
if True:
    ...
    sig = lr_importance
    batch_size = 128
    nepochs = 75#100#40#25
    height = 28
    width = 28
    nch = 1
    nb_classes = 10
    blmt = singlelevel_mt(sess, model, var_model, batch_size, lr_model, lr_importance, height, width, nch, nb_classes, rho, sig)
    importance_atan = np.ones((X_train.shape[0]))    
    ...
    for epoch in range(nepochs):
        ...
        for batch in range(nb_batches):
            ...
            ltr, lval, timp_atan = blmt.train(X_train[index_shuf[ind],:], Y_train[index_shuf[ind],:], X_val[ind_val,:], Y_val[ind_val,:], importance_atan[index_shuf[ind]])
        ...
        if epoch %1 == 0:
            print('epoch %d: loss_inner=%f loss_outer=%f'%(epoch, ltr, lval))
