...
def mult_lists(x,y):
    ...
    print("--- mu%s seconds ---" % (time.time() - tick))
    return tsum
...
class bilevel_mt(object):
    ...
    def __init__(self, sess, model, var_model, batch_size, lr_upper, lr_lower, height, width, nch, num_classes, rho=0., sig=0.):
        ...
        self.output_train = self.model.get_layer(self.x_train, self.model._get_softmax_name())
        print(self.model._get_logits_name())
        print(self.model._get_softmax_name())
        self.output_val= self.model.get_layer(self.x_val, self.model._get_softmax_name())
        ...
        print("--- a%s seconds ---" % (time.time() - tick))
        ...
        print("--- b%s seconds ---" % (time.time() - tick))
        
        tick = time.time() 
        self.loss_upper2 = self.rho*norm_sq_sum(tf.gradients(self.loss_lower,self.var_model))
        print("--- c%s seconds ---" % (time.time() - tick))
        
        tick = time.time() 
        dgdv = tf.gradients(self.loss_lower,self.var_model)
        print("--- 1c%s seconds ---" % (time.time() - tick))
        
        tick = time.time() 
        dfdv_ = tf.gradients(self.loss_upper1  , self.var_model, stop_gradients=self.var_model)
        print("--- d%s seconds ---" % (time.time() - tick))
        ...
        print("--- e%s seconds ---" % (time.time() - tick))
        ...
        print("--- f%s seconds ---" % (time.time() - tick))
        
        tick = time.time() 
        self.train_upper = tf.train.AdamOptimizer(self.lr_upper).minimize(self.loss_upper1 + self.loss_upper3,var_list=self.importance_atanh)
        print("--- g%s seconds ---" % (time.time() - tick))
