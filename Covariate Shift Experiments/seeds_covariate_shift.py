...
# Data Formatting 
data = pd.read_csv("seeds_data.csv",header=0)
mapping = {1.0 : 0, 2.0 : 1, 3.0 : 2}
data['h'] = data['h'].map(mapping)
features = list(data.columns[:7]) # Appending all the columns in feature vector
data_X = data[features].values
min_max_scaler = MinMaxScaler()
data_X = min_max_scaler.fit_transform(data_X)             
print(data_X[0])

features_Y = list(data.columns[7])
...
# Model
k_model = Sequential()
#k_model.add(Dense(input_dim=7, units=50))
k_model.add(Dense(input_dim=7, units=3))
...
x_tf = tf.placeholder(tf.float32, shape=(None, 7))
...
train_params = {
    ...
    'batch_size': 512,
    'learning_rate': 1E-3,
    'train_dir': os.path.join(*os.path.split(os.path.join("models", "seed_classifier"))[:-1]),
    'filename': os.path.split( os.path.join("models", "seed_classifier"))[-1]
}
...
for feature in range(features_t):
    ...
    for inde in range(times):
        ...
        if True:
            ...
            saver_model.save(sess,'./model_bilevel_seed_classifier.ckpt')
        ...
        if True:
            ...
            model_path = os.path.join('models', 'temp_seed_classifier')
