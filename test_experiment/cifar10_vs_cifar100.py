...
alpha_atanh = tf.Variable(np.zeros((batch_size), np.float32), name='alpha')
...
g = tf.reduce_mean(tf.multiply(alpha, tf.nn.softmax_cross_entropy_with_logits(logits = logits_inner, labels = y_train_tf))/ tf.sqrt(tf.reduce_sum(tf.square(alpha))))
#g = tf.reduce_mean(tf.multiply(alpha, tf.nn.softmax_cross_entropy_with_logits(logits = logits_inner, labels = y_train_tf)))/ tf.reduce_mean(alpha)

print(var_cls, "\n")
print(var_cls[0], var_cls[1], var_cls[2], var_cls[3], W4_tr, b4_tr)

dgdv = tf.gradients(g, [var_cls[0], var_cls[1], var_cls[2], var_cls[3], W4_tr, b4_tr])
gvnorm = 0.5 * rho * norm_sq_sum2(dgdv) 
lamb_g = lamb * g

logits_outer = cls_test
f1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = logits_outer, labels=y_test_tf))
f2 = beta * tf.reduce_mean(alpha)
f = f1 + f2
...
train_u = opt_u.minimize(loss, var_list = [alpha_atanh, W4_te, b4_te])
#print(var_cls)
train_v = opt_v.minimize(loss, var_list = [var_cls[0], var_cls[1], var_cls[2], var_cls[3], W4_tr, b4_tr,])
...
tgrad_and_var = opt_v.compute_gradients(loss, var_list=[var_cls[0], var_cls[1], var_cls[2], var_cls[3], W4_tr, b4_tr])
hvnorm = tf.reduce_sum([tf.reduce_sum(tf.square(t[0])) for t in tgrad_and_var])

tf.global_variables_initializer().run()
alpha_atan = np.zeros((X_train.shape[0]))  
# Normalize
alpha = 0.5*(np.tanh(alpha_atan)+1.)
#alpha = 0.5*alpha/np.mean(alpha)
alpha = 0.7 * alpha / np.sqrt(np.mean(np.square(alpha)))
...
for epoch in range(100):
    ...
    for batch in range(nb_batches):
        ...
        loss_t, f_t, g_t, gvnorm_t, lamb_g_t, tmp_alpha_atan, hunorm_t, hvnorm_t = ss.run([loss, f, g, gvnorm, lamb_g, alpha_atanh, hunorm, hvnorm], feed_dict = feed_dict)
    ...
    print("here1", epoch)
    print(hunorm_t**2 , hvnorm_t**2, hunorm_t**2 + hvnorm_t**2, eps_t)
    print("f = ", f_t)
    print("g = ", g_t)
    ...
    for i in range(1, 2001):
        correct[only_y[pts[-i]]] += 1

    print(alpha[pts[-2000:]])
    print(alpha[pts[1:]])
