...
class bilevel_penalty(object):
    ...
    def __init__(self,sess,f,g,u,v,lr_u,lr_v,rho_0,lamb_0,eps_0,nu_0,c_rho,c_lamb,c_eps):
        ...
        h_upper = f + self.gvnorm + self.gvnu
        h_lower = f + self.gvnorm + self.lamb_g + self.gvnu

        optim_u = tf.train.AdamOptimizer(self.lr_u_t)
        optim_v = tf.train.AdamOptimizer(self.lr_v_t)

        #print("computing min_v", len(v))
        self.min_v = optim_v.minimize(h_lower,var_list=self.v)
        #print("computing min_u", u.shape)
        self.min_u = optim_u.minimize(h_upper,var_list=self.u)
                
        #print("computing hunorm, hvnorm")
        tgrad_and_var = optim_u.compute_gradients(h_upper, var_list=self.u)
        self.hunorm = tf.reduce_sum([tf.reduce_sum(tf.square(t[0])) for t in tgrad_and_var])
        tgrad_and_var = optim_v.compute_gradients(h_lower, var_list=self.v)
