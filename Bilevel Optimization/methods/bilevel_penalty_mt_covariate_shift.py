...
class bilevel_cs(object):
    ...
    def __init__(self, sess, model, var_model, batch_size, lr_outer, lr_inner, features, num_classes, rho=0., sig=1.):
        ...
        self.f = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.output_val,labels=self.y_val))
        ...
        self.train_u = tf.train.AdamOptimizer(self.lr_outer).minimize(loss_u,var_list=self.importance_atan)
        self.train_v = tf.train.AdamOptimizer(self.lr_inner).minimize(loss_v,var_list=self.var_model)
    ...
    def train(self, x_train, y_train, x_val, y_val, importance_atan, rho_t, lamb_t, niter=1):
        ...
        l1, l2, l3, timp_atanh = self.sess.run([self.f, self.gvnorm, self.lamb_g, self.importance_atan],feed_dict=feed_dict)
