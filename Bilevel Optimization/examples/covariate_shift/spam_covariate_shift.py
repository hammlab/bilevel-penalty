...
# Data Formatting 
data = pd.read_csv("spam_data.csv", header=0)
mapping = {0.0 : 0, 1. : 1}
data['58'] = data['58'].map(mapping)
features = list(data.columns[:57]) 
...
data_Y = np.concatenate((np.ones(1813), np.zeros(2788)), axis = 0)
old_y = np.concatenate((np.ones(1813), np.zeros(2788)), axis = 0)
print(old_y[0])            
...
k_model.add(Dense(input_dim=57, units=2))
...
x_tf = tf.placeholder(tf.float32, shape=(None, 57))
...
train_params = {
    ...
    'batch_size': 512,
    'learning_rate': 1E-3,
    'train_dir': os.path.join(*os.path.split(os.path.join("models", "spam_classifier"))[:-1]),
    'filename': os.path.split( os.path.join("models", "spam_classifier"))[-1]
}
...
for feature in range(features_t):
    ...
    for inde in range(times):
        ...
        if True:
            ...
            saver_model.save(sess,'./model_bilevel_spam_classifier.ckpt')
        ...
        if True:
            ...
            model_path = os.path.join('models', 'temp_spam_classifier')
