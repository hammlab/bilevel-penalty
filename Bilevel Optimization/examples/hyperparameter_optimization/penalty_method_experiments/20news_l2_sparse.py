    ...
    for it_idx in range(times):
        ...
        for step in range(num_steps):
            ...
            if (step % 1 == 0):
                ...
                #pred_2 = clf.score(X_test_f, y_test_f)
                #print st_t, ", ", cost_1, ", ", cost_2, ", ", l4[0]
                
                #session.run(assign_test_labels, feed_dict={tf_test_labels_tf:y_test})
                #feed_dict = {tf_test_dataset: tf.SparseTensorValue(X_test_indices, X_test_data, X_test_shape)}
                #cost1 = test_loss.eval(feed_dict)
                #pred_1 = logits_test.eval(feed_dict)
                #in1 = np.argwhere(pred_1 >= 0.5).flatten()
                #in2 = np.argwhere(pred_1 < 0.5).flatten()
                #pred_1[in1] = 1
                #pred_1[in2] = 0 
                #acc1 = accuracy_score(y_test, pred_1)
                #acc1 = accuracy(pred_1, y_test)
