...
for line in f_f:
    entries = line.split(" ")
    Y_all_f.append(int(entries[0]))
    for ent in range(1, len(entries) - 1):
        entry = entries[ent]
        row_f.append(idx_f)
        item = entry.split(":")
        max_col_f = max(max_col_f, int(item[0]))
        col_f.append(item[0])
        data_f.append(item[1])
    idx_f+=1
    #print row
    #print col
    #print data
    #break
print "here"
row_f = np.array(row_f).astype(np.int)
col_f = np.array(col_f).astype(np.int)
data_f = np.array(data_f).astype(np.float)
X_all_f = csr_matrix((data_f,(row_f,col_f)), shape=(idx_f, max_col_f+1))
Y_all_f = np.array(Y_all_f)

shuff_idx_f = np.load("realsim_shuff_idx.npy")
...
shuff_idx = np.load("realsim_shuff_idx.npy")
