    ...
    for it in range(1, maxiter):
        ...
        while 1:
            ...
            if task_str.startswith(b'FG'):
                ...
                h_func, h_grad = h_func_grad(x, lambdak)
                try:
                    if linalg.norm(h_grad)  < epsilon_tol * norm_init * np.exp(np.min(old_lambdak) - np.min(lambda0)):
        ...
        if verbose > 0:
            ...
            print st_time, ",", t_loss, ",", g_func, ",", lambdak.sum(), lambdak.size, x.size, it, maxiter
            cum_time += st_time
            loss_total[it] += t_loss
            loss_val_total[it] += g_func
