...
class MultiLogisticRegressionCV(linear_model.base.BaseEstimator,
    ...
    def __init__(self, alpha0=None, tol=0.1, callback=None, verbose=0,
                 tolerance_decrease='exact', max_iter=5):
        self.alpha0 = alpha0
        self.tol = tol
    ...
    def fit(self, Xt, yt, Xh, yh, X_test = None, y_test = None, callback=None):
        ...
        def test_loss(x, alpha):
            #return _multinomial_loss(x, X_test, Ytest_multi, np.zeros(alpha.size), sample_weight_t)[0]
            return _multinomial_loss_grad(
                x, X_test, Ytest_multi, np.zeros(alpha.size),
                sample_weight_t)[:1]
        ...
        print "initial:", test_loss(x0, np.zeros((n_classes, n_features)))
        opt = hoag_lbfgs(
            test_loss, h_func_grad, h_hessian, h_crossed, g_func_grad, x0,
            callback=callback,
